...
# UI wrapper: keep all V2 model/business logic in core_app.py and only replace presentation.
_original_title = st.title
_original_caption = st.caption
_original_sidebar_header = st.sidebar.header
_caption_count = {"n": 0}
_sidebar_header_count = {"n": 0}
...
def _competition_caption(body, *args, **kwargs):
    _caption_count["n"] += 1
    # Replace the old top caption with a visual five-step competition workflow.
...
def _competition_sidebar_header(body, *args, **kwargs):
    _sidebar_header_count["n"] += 1
    if _sidebar_header_count["n"] == 1:
        st.sidebar.markdown(
            """
<div class="sidebar-brand">
  ...
  <div class="sb-note">先定义地块与番茄生育阶段，再由模型生成可执行的水凝胶处方与水肥管理方案。</div>
    ...
    return _original_sidebar_header(body, *args, **kwargs)


st.title = _competition_title
st.caption = _competition_caption
st.sidebar.header = _competition_sidebar_header
